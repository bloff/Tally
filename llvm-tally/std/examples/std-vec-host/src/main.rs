...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let entry = manager.load_function(&workload, "run_std_vec_workload")?;
    ...
    println!("thread,target_len,len,sum,charges,state,error");
    for (id, args) in workload_args.iter().enumerate() {
        let stats = manager.stats_for_thread(id)?;
        println!(
            "{},{},{},{},{},{:?},{:?}",
            id, args.target_len, args.len, args.sum, stats.charges, stats.state, stats.error
        );
    }
